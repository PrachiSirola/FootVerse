...
export function transformCJLive(raw, category = "", subcategory = "") {
  ...
  const productId = raw.id || raw.pid || raw.productId || "";
  ...
  // productId seeds the stable 50/50 split for gender-neutral products so a
  // product never flips category between refreshes.
  const resolvedCategory = classifyCategory(name, category, productId);
